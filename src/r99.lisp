...
(define-easy-handler (problems :uri "/problems") ()
  (let ((results
    ...
    (page
     ...
     (:h1 :class "blue" "WARNING")
     (:p :class "blue" "回答にならない答を一旦提出、他人の回答をコピーし、自分の回答としてアップデートするの、やめよう。発覚しないと思っていたら大間違い。")
     (:p :class "blue" "正真正銘自分のプログラムでも、動作確認できてないプログラムはゴミです。")
     (:hr)
     (loop for row = (dbi:fetch results)
           while row
           do
